...
if __name__ == "__main__":
    ...
    for i in range(how_many_citadels):
        ...
        pos = find_image_on_screen("images\\go4k.png", cord_menu_button_go_citadels)#Search for go button on citadels list
        if pos is None or len(pos) == 0:
            if verify_store_screen(): #if store open, verify and return True if store is closed or false if was false positive
                posb = find_image_on_screen("images\\go4k.png",
                ...
                if posb is None or len(posb) == 0:
                    ...
                    click(screen_area[0] + posb[0] + 15, screen_area[1] + posb[1] + 15)
            else:
            ...
            if verify_store_screen():
                ...
                click(center_of_screen[0], center_of_screen[1])
            result = find_image_on_screen("images\\watchtower\\cursedcitadel4k.png.", verify_if_open_citadel)
            if result is None or len(result) == 0:
                if verify_store_screen():
                ...
                click(cord_attack_button[0], cord_attack_button[1])
                pyautogui.move(-200, 0) #position curson inside of troops selection
                ...
                time.sleep(2.0)  # Wait before clicking speed up, as sometimes the game takes a while to create the button
                click(cord_speedup_march[0], cord_speedup_march[1])  # click speedup
                result = find_image_on_screen("images\\troopsonthemarch4k.png.",cord_click_use_speedups_screen)  # Check if the
                # acceleration screen is really for the march to citadel
                if result is None or len(result) == 0:
                    print("Generic Error. Try again after 10 minutes")
                    errors = errors + 1
                    counter = counter - 1
                    time.sleep(600.0)
                    #break
